<?php
	...
	if(isset($_REQUEST['show_user_id'])) { $show_user_id = $_REQUEST['show_user_id'];
        ...
        ?>
        <form class="poppins pd-20" name="user_form" method="POST">
            ...
            <div class="row p-3">
                ...
                <div class="col-lg-3 col-md-4 col-12 py-2 px-lg-1">
                    ...
                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" value="" id="checkDefault" name="is_technician" onchange="Javascript:checkisTechnician();"  value="<?php if ($is_technician == '1') { echo '1';   } else { echo '2'; } ?>" <?php if ($is_technician == '1') { ?> checked <?php } ?>>
                        <label class="form-check-label smallfnt" for="checkDefault">Is Technician</label>
                    </div>
                </div>
                <div class="col-lg-3 col-md-4 col-12 py-2 px-lg-1">
                    <div class="form-group">
                ...
                <?php if(empty($admin)) { ?>
                    <div class="col-lg-3 col-md-4 col-12 py-2 px-lg-1">
                        <div class="form-group">
                            <div class="form-label-group in-border">
                                <select class="select2 select2-danger" name="role_id" data-dropdown-css-class="select2-danger" style="width: 100%;">
                ...
                <div class="col-md-12 pt-3 text-center">
                    <button class="btn btn-danger submit_button" type="button" onClick="Javascript:SaveModalContent('user_form', 'user_changes.php', 'user.php');">
                        Submit
                    </button>
                </div>
            </div>
    ...
    if(isset($_POST['edit_id'])) {	
        ...
        $valid_user = ""; $form_name = "user_form"; $is_technician = 2; 
        ...
        if(isset($_POST['name'])) {
            $name = $_POST['name'];
            $name = trim($name);
            $name_error = $valid->valid_party_name($name,'name','1','25');
        }
        ...
        if(isset($_POST['is_technician'])) {
            $is_technician = $_POST['is_technician'];
            $is_technician = trim($is_technician);
        }
        ...
        if(empty($valid_user)) {
            ...
            if(preg_match("/^\d+$/", $check_user_id_ip_address)) {
                ...
                $login_id = ""; $lower_case_name = "";
                if(!empty($username)) {
                    // $login_id = $username;
                    $lower_case_name = strtolower($username);
                ...
                if(!empty($lower_case_name)) {
                    $prev_user_id = $obj->CheckUserIDAlreadyExists($lower_case_name);
                ...
                if(empty($edit_id)) {
                    if(empty($prev_user_id)) {
                        ...
                        $columns = array('created_date_time', 'creator', 'creator_name', 'user_id', 'name', 'mobile_number', 'role_id', 'password', 'admin', 'deleted', 'is_technician', 'username', 'lower_case_name', 'bill_company_id');
                        $values = array("'".$created_date_time."'", "'".$creator."'", "'".$creator_name."'", "'".$null_value."'", "'".$name."'", "'".$mobile_number."'", "'".$role_id."'", "'".$password."'", "'".$admin."'", "'0'", "'".$is_technician."'", "'".$username."'", "'".$lower_case_name."'", "'".$bill_company_id."'");
                    ...
                    if(empty($prev_user_id) || $prev_user_id == $edit_id) {
                        ...
                        if(preg_match("/^\d+$/", $getUniqueID)) {
                            ...
                            $columns = array(); $values = array();						
                            $columns = array('creator_name', 'name', 'mobile_number', 'role_id', 'password', 'is_technician', 'username', 'lower_case_name');
                            $values = array("'".$creator_name."'", "'".$name."'", "'".$mobile_number."'", "'".$role_id."'", "'".$password."'", "'".$is_technician."'","'".$username."'", "'".$lower_case_name."'");
        ...
        if(!empty($search_text)) {
            ...
            if(!empty($total_records_list)) {
                foreach ($total_records_list as $val) {
                    if((strpos(strtolower(html_entity_decode($obj->encode_decode('decrypt', $val['name']))), $search_text) !== false)) {
                        $list[] = $val;
                    }
                }
            }
                            ...
                            if(!empty($prefix)) { $index = $index + $prefix; } 
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        /* if(!empty($show_creator_name_for_all_screens) && $show_creator_name_for_all_screens =='ON'){ */
                                            ?>
    ...
    if(isset($_REQUEST['delete_user_id'])) {
        ...
        if(!empty($delete_user_id)) {	
            ...
            if(preg_match("/^\d+$/", $user_unique_id)) {
                ...
                if(empty($admin)) {
                    $name = "";
                    $name = $obj->getTableColumnValue($GLOBALS['user_table'], 'user_id', $delete_user_id, 'name');
                
                    $action = "";
                    if(!empty($name)) {
                        $action = "User Deleted. Details - ".$obj->encode_decode('decrypt', $name);
                    }
