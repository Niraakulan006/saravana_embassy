...
function getCities(form_name, district, city, context) {
	console.log(form_name + " / " + district + "/" + city)
	// Scope to modal or whole page
	...
	let districtElement = formScope.find('select[name="district"]')[0];
	let cityElement = formScope.find('select[name="city"]')[0];

	if (districtElement && cityElement) {
		let selectedDistrictIndex = districtElement.selectedIndex;
		let selected_district = districtElement.value;
		let selected_state = stateElement ? stateElement.value : "";

		// default: get district from array
		let districtValue = city_district_arr[selectedDistrictIndex - 1];
		// override if not Tamil Nadu
		if (selected_state !== "Tamil Nadu") {
			districtValue = selected_district;
		}
		...
		if (districtValue === selected_district) {
			// handle "Others"
			if (city === 'Others') {
				if ($('#others_city_cover').hasClass('d-none')) {
					$('#others_city_cover').removeClass('d-none');
				}
				let post_url = "dashboard_changes.php?check_login_session=1";
				$.ajax({
					url: post_url,
					success: function (check_login_session) {
						if (check_login_session == 1) {
							let post_url = "common_changes.php?others_city=1&selected_district=" + selectedDistrictIndex + "&form_name=" + form_name;
							$.ajax({
								url: post_url,
								success: function (result) {
									if ($('#others_city_cover').length > 0) {
										$('#others_city_cover').html(result);
									}
								}
							});
						} else {
				...
				if (!$('#others_city_cover').hasClass('d-none')) {
					$('#others_city_cover').addClass('d-none');
				}
				$('input[name="others_city"]').val('');
			}

			// Tamil Nadu cities
			if (selected_state === "Tamil Nadu") {
				if (typeof city_index[selectedDistrictIndex] !== 'undefined' && city_index[selectedDistrictIndex] !== null) {
					let city_arr = city_index[selectedDistrictIndex].split("|");
					let others_city = [];

					let post_url = "common_changes.php?get_city=1&get_district=" + districtValue;
					$.ajax({
						url: post_url,
						success: function (result) {
							result = result.replace('\r\n', '').trim();
							if (result !== '' && result !== 'NULL') {
								others_city = result.split(',');
								for (let i = 0; i < others_city.length; i++) {
									others_city[i] = others_city[i].replace('\r\n', '');
									if (!city_arr.includes(others_city[i])) {
										city_arr.push(others_city[i]);
									}
