var city_district_arr = new Array("Others", "Ariyalur", "Chengalpattu", "Chennai", "Coimbatore", "Cuddalore", "Dharmapuri", "Dindigul", "Erode", "Kallakurichi", "Kanchipuram", "Kanniyakumari", "Karur", "Krishnagiri", "Madurai", "Mayiladuthurai", "Nagapattinam", "Namakkal", "Perambalur", "Pudukkottai", "Ramanathapuram", "Ranipet", "Salem", "Sivaganga", "Tenkasi", "Thanjavur", "Theni", "The Nilgiris", "Thiruvallur", "Thiruvarur", "Tiruchirappalli", "Tirunelveli", "Tirupathur", "Tiruppur", "Tiruvannamalai", "Tuticorin", "Vellore", "Villupuram", "Virudhunagar");
...
city_index[33]="Others|Avanashi|Dharapuram|Kangeyam|Madathukulam|Palladam|Tiruppur North|Tiruppur South|Udumalaipettai|Uthukuli";
...
function getCities(district, city) {
	//console.log('district - '+district+', city - '+city);
	if (jQuery('form[name="customer_form"]').find('select[name="city"]').length > 0) {
		var selectedDistrictIndex = document.forms["customer_form"]["district"].selectedIndex;
		var district = city_district_arr[selectedDistrictIndex - 1];
		var selected_district = document.forms["customer_form"]["district"].value;
		var cityElement = document.forms["customer_form"]["city"];
		cityElement.length = 0;
		cityElement.options[0] = new Option('Select', '');
		cityElement.selectedIndex = 0;
		//console.log(city_district_arr);
		//console.log('selectedDistrictIndex - '+selectedDistrictIndex+', district - '+district+', selected_district - '+selected_district);
		if (district == selected_district) {
			var city_arr = city_index[selectedDistrictIndex - 1].split("|");
	...
	if (jQuery('form[name="shipping_form"]').find('select[name="city"]').length > 0) {
		var selectedDistrictIndex = document.forms["shipping_form"]["district"].selectedIndex;
		var district = city_district_arr[selectedDistrictIndex - 1];
		var selected_district = document.forms["shipping_form"]["district"].value;
		var cityElement = document.forms["shipping_form"]["city"];
		cityElement.length = 0;
		cityElement.options[0] = new Option('Select', '');
		cityElement.selectedIndex = 0;
		//console.log(city_district_arr);
	...
	if (jQuery('form[name="billing_form"]').find('select[name="bill_city"]').length > 0) {
		var selectedDistrictIndex = document.forms["billing_form"]["bill_district"].selectedIndex;
		var district = city_district_arr[selectedDistrictIndex - 1];
		var selected_district = document.forms["billing_form"]["bill_district"].value;
		var cityElement = document.forms["billing_form"]["bill_city"];
		cityElement.length = 0;
		cityElement.options[0] = new Option('Select', '');
		cityElement.selectedIndex = 0;
		//console.log(city_district_arr);
